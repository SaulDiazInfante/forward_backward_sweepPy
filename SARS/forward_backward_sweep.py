import numpy as np
from sars import OptimalControlProblem
...
class ForwardBackwardSweep(OptimalControlProblem):
    ...
    def __init__(self, eps=.00001, n_max=365):
        ...
        # s elf.u = np.ones([n_max, con_dim]) * ([self.u_1_lower,
        # self.u_2_lower])
        self.u = np.zeros([n_max, con_dim])
        self.u[:, 0] = 0.2 * np.ones(n_max)
        self.u[:, 1] = 0.2 * np.ones(n_max)
        self.lambda_adjoint = np.zeros([n_max, dyn_dim])
        self.j_cost = np.zeros(n_max)
    
    def runge_kutta_forward(self, u):
        x_0 = np.array([self.s_zero, self.e_zero, self.q_zero,
                        self.i_zero, self.j_zero, self.r_zero])
        h = self.h
        n_max = self.n_max
        dyn_dim = self.dynamics_dim
        con_dim = self.control_dim
