import numpy as np
from example_14_1 import OptimalControlProblem
...
class ForwardBackwardSweep(OptimalControlProblem):
    ...
    def __init__(self, eps=.01, n_max=10000):
        """
    ...
    def runge_kutta_forward(self, u):
        x_0 = np.array([self.t_cell_zero, self.t_cell_infected_zero,
                        self.virus_particle_zero])
        h = self.h
        n_max = self.n_max
    ...
    def runge_kutta_backward(self, x, u):
        ...
        for j in np.arange(n_max - 1, 0, -1):
            ...
            k_1 = self.lambda_f(x_j, u_j, lambda_j)
            k_2 = self.lambda_f(x_mj, u_mj, lambda_j - 0.5 * h * k_1)
            k_3 = self.lambda_f(x_mj, u_mj, lambda_j - 0.5 * h * k_2)
            k_4 = self.lambda_f(x_jm1, u_jm1, lambda_j - h * k_3)
