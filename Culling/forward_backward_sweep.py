import numpy as np
from culling_quadratic import QuadraticOptimalControlProblem
from culling_linear import LinearOptimalControlProblem
...
class ForwardBackwardSweepQuadratic(QuadraticOptimalControlProblem):
    
    def __init__(self, eps=eps, n_max=n_max):
        """
        :type t_0: initial time
        """
        #
        super(ForwardBackwardSweepQuadratic, self).__init__()
        self.n_max = n_max
        self.eps = eps
    ...
    def runge_kutta_forward(self, u):
        x_0 = np.array([self.s_zero, self.i_zero])
        h = self.h
        n_max = self.n_max
        dyn_dim = self.dynamics_dim
        con_dim = self.control_dim
    ...
    def control_cost(self, x_k, u_k):
        ...
        for t in np.arange(n_max - 1):
            j_cost_i = i[t] + p * (u[t] ** theta)
            j_cost[t + 1] = j_cost[t] + j_cost_i * h
        
        self.j_cost = j_cost
        return j_cost


class ForwardBackwardSweepLinear(LinearOptimalControlProblem):
    ...
    def __init__(self, eps=eps, n_max=n_max):
        ...
        """
        #
        super(ForwardBackwardSweepLinear, self).__init__()
        self.n_max = n_max
        self.eps = eps
        self.t = np.linspace(self.t_0, self.t_f, n_max)
        self.h = self.t[1] - self.t[0]
    ...
    def forward_backward_sweep(self):
        ...
        while flag:
            ...
            cont = cont + 1
            print cont, test
        u = self.optimality_condition(x, u, lambda_, n_max)
        return [x, lambda_, u]
